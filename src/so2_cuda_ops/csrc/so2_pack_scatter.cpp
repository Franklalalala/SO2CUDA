...
torch::Tensor pack_pairs_multi_desc_fp32_cuda(
    torch::Tensor x,
    torch::Tensor wigner,
    torch::Tensor desc,
    torch::Tensor offsets,
...
torch::Tensor scatter_raw_pairs_multi_output_major_add_forward_fp32_cuda(
    std::vector<torch::Tensor> raws,
    torch::Tensor wigner,
    torch::Tensor offsets,
    torch::Tensor compact_offsets,
...
std::vector<torch::Tensor> raw_pairs_multi_output_grad_fp32_cuda(
    torch::Tensor grad_out,
    torch::Tensor wigner,
    std::vector<torch::Tensor> out_bases,
    std::vector<torch::Tensor> out_ls,
...
torch::Tensor block_complex_forward_fp32_cuda(
    torch::Tensor pair,
    torch::Tensor weight);

std::vector<torch::Tensor> block_complex_backward_fp32_cuda(
    torch::Tensor grad_out,
    torch::Tensor pair,
    torch::Tensor weight);
...
torch::Tensor scatter_m0_forward_fp32_cuda(
    torch::Tensor y_m0,
    torch::Tensor wigner,
    torch::Tensor out_base,
    torch::Tensor out_l,
...
torch::Tensor pack_pairs_multi_desc_fp32(
    torch::Tensor x,
    torch::Tensor wigner,
    torch::Tensor desc,
    torch::Tensor offsets,
    ...
    int64_t wigner_stride) {
  check_cuda_contiguous(x, "x");
  check_cuda_contiguous(desc, "desc");
  check_cuda_contiguous(offsets, "offsets");
  check_cuda_contiguous(compact_offsets, "compact_offsets");
  TORCH_CHECK(x.scalar_type() == torch::kFloat32, "x must be fp32");
  TORCH_CHECK(desc.scalar_type() == torch::kInt64, "desc must be int64");
  TORCH_CHECK(offsets.scalar_type() == torch::kInt64, "offsets must be int64");
  TORCH_CHECK(compact_offsets.scalar_type() == torch::kInt64, "compact_offsets must be int64");
  TORCH_CHECK(x.dim() == 2, "x must be [N, in_dim]");
  TORCH_CHECK(desc.dim() == 2 && desc.size(1) == 3, "desc must be [total_cin, 3]");
  if (rotate_in) {
    check_cuda_contiguous(wigner, "wigner");
    TORCH_CHECK(wigner.scalar_type() == torch::kFloat32, "wigner must be fp32");
  }
  return pack_pairs_multi_desc_fp32_cuda(
      x, wigner, desc, offsets, compact_offsets, rotate_in, wigner_mode, wigner_stride);
}
...
torch::Tensor scatter_pairs_multi_output_major_forward_fp32(
    std::vector<torch::Tensor> pairs,
    torch::Tensor wigner,
    torch::Tensor offsets,
    torch::Tensor compact_offsets,
    ...
    int64_t wigner_mode,
    int64_t wigner_stride) {
  TORCH_CHECK(!pairs.empty(), "pairs must be non-empty");
  check_cuda_contiguous(offsets, "offsets");
  check_cuda_contiguous(compact_offsets, "compact_offsets");
  check_cuda_contiguous(cout_prefix, "cout_prefix");
  check_cuda_contiguous(m_values, "m_values");
  ...
  TORCH_CHECK(entry_d.scalar_type() == torch::kInt64, "entry_d must be int64");
  TORCH_CHECK(entry_l.scalar_type() == torch::kInt64, "entry_l must be int64");
  TORCH_CHECK(cout_prefix.numel() == static_cast<int64_t>(pairs.size()) + 1,
              "cout_prefix must have pair_count + 1 entries");
  TORCH_CHECK(m_values.numel() == static_cast<int64_t>(pairs.size()),
              "m_values must have pair_count entries");
  TORCH_CHECK(entry_offsets.numel() == out_dim + 1, "entry_offsets must be [out_dim + 1]");
  TORCH_CHECK(entry_m.numel() == entry_channel.numel() &&
              entry_m.numel() == entry_d.numel() &&
              entry_m.numel() == entry_l.numel(),
  ...
  return scatter_pairs_multi_output_major_forward_fp32_cuda(
      pairs, wigner, offsets, compact_offsets, cout_prefix, m_values,
      entry_offsets, entry_m, entry_channel, entry_d, entry_l,
      out_dim, rotate_out, wigner_mode, wigner_stride);
}

torch::Tensor scatter_raw_pairs_multi_output_major_add_forward_fp32(
    std::vector<torch::Tensor> raws,
    torch::Tensor wigner,
    torch::Tensor offsets,
    torch::Tensor compact_offsets,
  ...
  TORCH_CHECK(out.dim() == 2, "out must be [N, out_dim]");
  TORCH_CHECK(offsets.scalar_type() == torch::kInt64, "offsets must be int64");
  TORCH_CHECK(compact_offsets.scalar_type() == torch::kInt64, "compact_offsets must be int64");
  TORCH_CHECK(cout_prefix.scalar_type() == torch::kInt64, "cout_prefix must be int64");
  TORCH_CHECK(m_values.scalar_type() == torch::kInt64, "m_values must be int64");
  ...
  TORCH_CHECK(m_values.numel() == static_cast<int64_t>(raws.size()),
              "m_values must have raw_count entries");
  TORCH_CHECK(entry_offsets.numel() == out.size(1) + 1, "entry_offsets must be [out_dim + 1]");
  TORCH_CHECK(entry_m.numel() == entry_channel.numel() &&
              entry_m.numel() == entry_d.numel() &&
              entry_m.numel() == entry_l.numel(),
              "entry arrays must have the same length");
  ...
  return scatter_raw_pairs_multi_output_major_add_forward_fp32_cuda(
      raws, wigner, offsets, compact_offsets, cout_prefix, m_values,
      entry_offsets, entry_m, entry_channel, entry_d, entry_l,
      out, rotate_out, wigner_mode, wigner_stride);
}
...
std::vector<torch::Tensor> raw_pairs_multi_output_grad_fp32(
    torch::Tensor grad_out,
    torch::Tensor wigner,
    std::vector<torch::Tensor> out_bases,
    std::vector<torch::Tensor> out_ls,
    ...
    int64_t wigner_mode,
    int64_t wigner_stride) {
  TORCH_CHECK(!out_bases.empty(), "out_bases must be non-empty");
  TORCH_CHECK(out_bases.size() == out_ls.size(), "out_bases and out_ls must have the same length");
  check_cuda_contiguous(grad_out, "grad_out");
  check_cuda_contiguous(offsets, "offsets");
  check_cuda_contiguous(compact_offsets, "compact_offsets");
  check_cuda_contiguous(cout_prefix, "cout_prefix");
  ...
  TORCH_CHECK(m_values.scalar_type() == torch::kInt64, "m_values must be int64");
  TORCH_CHECK(grad_out.dim() == 2, "grad_out must be [N, out_dim]");
  TORCH_CHECK(cout_prefix.numel() == static_cast<int64_t>(out_bases.size()) + 1,
              "cout_prefix must have raw_count + 1 entries");
  TORCH_CHECK(m_values.numel() == static_cast<int64_t>(out_bases.size()),
              "m_values must have raw_count entries");
  for (size_t i = 0; i < out_bases.size(); ++i) {
    check_cuda_contiguous(out_bases[i], "out_base");
    check_cuda_contiguous(out_ls[i], "out_l");
    TORCH_CHECK(out_bases[i].scalar_type() == torch::kInt64, "out_base must be int64");
    TORCH_CHECK(out_ls[i].scalar_type() == torch::kInt64, "out_l must be int64");
    TORCH_CHECK(out_bases[i].dim() == 1 && out_ls[i].dim() == 1 &&
                out_bases[i].numel() == out_ls[i].numel(),
                "output maps must be 1D and aligned");
  }
  if (rotate_out) {
    check_cuda_contiguous(wigner, "wigner");
  ...
  return raw_pairs_multi_output_grad_fp32_cuda(
      grad_out, wigner, out_bases, out_ls, offsets, compact_offsets,
      cout_prefix, m_values, rotate_out, wigner_mode, wigner_stride);
}

std::vector<torch::Tensor> m0_raw_pairs_multi_output_grad_fp32(
    torch::Tensor grad_out,
    torch::Tensor wigner,
    torch::Tensor m0_out_base,
    torch::Tensor m0_out_l,
  ...
  TORCH_CHECK(out_bases.size() == out_ls.size(), "out_bases and out_ls must have the same length");
  check_cuda_contiguous(grad_out, "grad_out");
  check_cuda_contiguous(m0_out_base, "m0_out_base");
  check_cuda_contiguous(m0_out_l, "m0_out_l");
  check_cuda_contiguous(offsets, "offsets");
  check_cuda_contiguous(compact_offsets, "compact_offsets");
  check_cuda_contiguous(cout_prefix, "cout_prefix");
  check_cuda_contiguous(m_values, "m_values");
  TORCH_CHECK(grad_out.scalar_type() == torch::kFloat32, "grad_out must be fp32");
  TORCH_CHECK(m0_out_base.scalar_type() == torch::kInt64, "m0_out_base must be int64");
  TORCH_CHECK(m0_out_l.scalar_type() == torch::kInt64, "m0_out_l must be int64");
  TORCH_CHECK(offsets.scalar_type() == torch::kInt64, "offsets must be int64");
  TORCH_CHECK(compact_offsets.scalar_type() == torch::kInt64, "compact_offsets must be int64");
  TORCH_CHECK(cout_prefix.scalar_type() == torch::kInt64, "cout_prefix must be int64");
  TORCH_CHECK(m_values.scalar_type() == torch::kInt64, "m_values must be int64");
  TORCH_CHECK(grad_out.dim() == 2, "grad_out must be [N, out_dim]");
  TORCH_CHECK(m0_out_base.dim() == 1 && m0_out_l.dim() == 1 &&
              m0_out_base.numel() == m0_out_l.numel(),
              "m0 output maps must be 1D and aligned");
  TORCH_CHECK(cout_prefix.numel() == static_cast<int64_t>(out_bases.size()) + 1,
              "cout_prefix must have raw_count + 1 entries");
  TORCH_CHECK(m_values.numel() == static_cast<int64_t>(out_bases.size()),
              "m_values must have raw_count entries");
  ...
  return m0_raw_pairs_multi_output_grad_fp32_cuda(
      grad_out, wigner, m0_out_base, m0_out_l, out_bases, out_ls,
      offsets, compact_offsets, cout_prefix, m_values,
      rotate_out, wigner_mode, wigner_stride);
}

std::vector<torch::Tensor> m0_raw_pairs_multi_output_grad_flat_fp32(
    torch::Tensor grad_out,
    torch::Tensor wigner,
    torch::Tensor m0_out_base,
    torch::Tensor m0_out_l,
    ...
    int64_t wigner_mode,
    int64_t wigner_stride) {
  TORCH_CHECK(!cout_values.empty(), "cout_values must be non-empty");
  check_cuda_contiguous(grad_out, "grad_out");
  check_cuda_contiguous(m0_out_base, "m0_out_base");
  check_cuda_contiguous(m0_out_l, "m0_out_l");
  check_cuda_contiguous(out_base_all, "out_base_all");
  check_cuda_contiguous(out_l_all, "out_l_all");
  check_cuda_contiguous(offsets, "offsets");
  check_cuda_contiguous(compact_offsets, "compact_offsets");
  check_cuda_contiguous(cout_prefix, "cout_prefix");
  check_cuda_contiguous(m_values, "m_values");
  ...
  TORCH_CHECK(m0_out_base.scalar_type() == torch::kInt64, "m0_out_base must be int64");
  TORCH_CHECK(m0_out_l.scalar_type() == torch::kInt64, "m0_out_l must be int64");
  TORCH_CHECK(out_base_all.scalar_type() == torch::kInt64, "out_base_all must be int64");
  TORCH_CHECK(out_l_all.scalar_type() == torch::kInt64, "out_l_all must be int64");
  TORCH_CHECK(offsets.scalar_type() == torch::kInt64, "offsets must be int64");
  TORCH_CHECK(compact_offsets.scalar_type() == torch::kInt64, "compact_offsets must be int64");
  TORCH_CHECK(cout_prefix.scalar_type() == torch::kInt64, "cout_prefix must be int64");
  TORCH_CHECK(m_values.scalar_type() == torch::kInt64, "m_values must be int64");
  ...
  TORCH_CHECK(m0_out_base.dim() == 1 && m0_out_l.dim() == 1 &&
              m0_out_base.numel() == m0_out_l.numel(),
              "m0 output maps must be 1D and aligned");
  TORCH_CHECK(out_base_all.dim() == 1 && out_l_all.dim() == 1 &&
              out_base_all.numel() == out_l_all.numel(),
              "flattened output maps must be 1D and aligned");
  TORCH_CHECK(cout_prefix.numel() == static_cast<int64_t>(cout_values.size()) + 1,
              "cout_prefix must have raw_count + 1 entries");
  TORCH_CHECK(m_values.numel() == static_cast<int64_t>(cout_values.size()),
              "m_values must have raw_count entries");
  int64_t total_cout = 0;
  for (const int64_t cout : cout_values) {
    TORCH_CHECK(cout >= 0, "cout_values must be non-negative");
    total_cout += cout;
  }
  TORCH_CHECK(total_cout == out_base_all.numel(), "cout_values must sum to flattened output map length");
  if (rotate_out) {
    check_cuda_contiguous(wigner, "wigner");
    TORCH_CHECK(wigner.scalar_type() == torch::kFloat32, "wigner must be fp32");
  }
  return m0_raw_pairs_multi_output_grad_flat_fp32_cuda(
      grad_out, wigner, m0_out_base, m0_out_l, out_base_all, out_l_all,
      offsets, compact_offsets, cout_prefix, m_values, cout_values,
      rotate_out, wigner_mode, wigner_stride);
}
...
torch::Tensor scatter_m0_forward_fp32(
    torch::Tensor y_m0,
    torch::Tensor wigner,
    torch::Tensor out_base,
    torch::Tensor out_l,
    ...
    int64_t wigner_mode,
    int64_t wigner_stride) {
  check_cuda_contiguous(y_m0, "y_m0");
  check_cuda_contiguous(out_base, "out_base");
  check_cuda_contiguous(out_l, "out_l");
  check_cuda_contiguous(offsets, "offsets");
  check_cuda_contiguous(compact_offsets, "compact_offsets");
  TORCH_CHECK(y_m0.scalar_type() == torch::kFloat32, "y_m0 must be fp32");
  TORCH_CHECK(out_base.scalar_type() == torch::kInt64, "out_base must be int64");
  TORCH_CHECK(out_l.scalar_type() == torch::kInt64, "out_l must be int64");
  TORCH_CHECK(offsets.scalar_type() == torch::kInt64, "offsets must be int64");
  TORCH_CHECK(compact_offsets.scalar_type() == torch::kInt64, "compact_offsets must be int64");
  TORCH_CHECK(y_m0.dim() == 2, "y_m0 must be [N, Cout]");
  TORCH_CHECK(out_base.dim() == 1 && out_l.dim() == 1 && out_base.numel() == out_l.numel(),
              "output maps must be 1D and aligned");
  TORCH_CHECK(y_m0.size(1) == out_base.numel(), "y_m0 Cout mismatch");
  TORCH_CHECK(out_dim > 0, "out_dim must be positive");
  if (rotate_out) {
    check_cuda_contiguous(wigner, "wigner");
    TORCH_CHECK(wigner.scalar_type() == torch::kFloat32, "wigner must be fp32");
  }
  return scatter_m0_forward_fp32_cuda(
      y_m0, wigner, out_base, out_l, offsets, compact_offsets,
      out_dim, rotate_out, wigner_mode, wigner_stride);
}
...
torch::Tensor block_complex_forward_fp32(
    torch::Tensor pair,
    torch::Tensor weight) {
  check_cuda_contiguous(pair, "pair");
  ...
  TORCH_CHECK(pair.dim() == 3 && pair.size(1) == 2, "pair must be [N, 2, Cin]");
  TORCH_CHECK(weight.dim() == 3 && weight.size(0) == 1, "weight must be [1, 2*Cout, Cin]");
  TORCH_CHECK(weight.size(1) % 2 == 0, "weight output dimension must be even");
  TORCH_CHECK(weight.size(2) == pair.size(2), "weight Cin must match pair Cin");
  return block_complex_forward_fp32_cuda(pair, weight);
}

std::vector<torch::Tensor> block_complex_backward_fp32(
    torch::Tensor grad_out,
    torch::Tensor pair,
    torch::Tensor weight) {
  check_cuda_contiguous(grad_out, "grad_out");
  ...
  TORCH_CHECK(weight.dim() == 3 && weight.size(0) == 1, "weight must be [1, 2*Cout, Cin]");
  TORCH_CHECK(weight.size(1) % 2 == 0, "weight output dimension must be even");
  const int64_t cout = weight.size(1) / 2;
  TORCH_CHECK(weight.size(2) == pair.size(2), "weight Cin must match pair Cin");
  TORCH_CHECK(grad_out.dim() == 3 && grad_out.size(0) == pair.size(0) &&
              grad_out.size(1) == 2 && grad_out.size(2) == cout,
              "grad_out must be [N, 2, Cout]");
  return block_complex_backward_fp32_cuda(grad_out, pair, weight);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("pack_pairs_multi_desc_fp32", &pack_pairs_multi_desc_fp32, "SO2 descriptor-based grouped pack pair fp32");
  m.def("scatter_m0_forward_fp32", &scatter_m0_forward_fp32, "SO2 m0 output scatter forward fp32");
  ...
  m.def("scatter_raw_pairs_multi_output_major_add_forward_fp32", &scatter_raw_pairs_multi_output_major_add_forward_fp32, "SO2 output-major grouped raw pair epilogue add-into-output forward fp32");
  m.def("scatter_pairs_multi_output_major_forward_fp32", &scatter_pairs_multi_output_major_forward_fp32, "SO2 output-major grouped finished pair epilogue forward fp32");
  m.def("raw_pair_output_grad_fp32", &raw_pair_output_grad_fp32, "SO2 MoE fused P0 raw pair output grad fp32");
  m.def("raw_pairs_multi_output_grad_fp32", &raw_pairs_multi_output_grad_fp32, "SO2 fused multi raw pair output grad fp32");
  m.def("m0_raw_pairs_multi_output_grad_fp32", &m0_raw_pairs_multi_output_grad_fp32, "SO2 fused m0 plus multi raw pair output grad fp32");
